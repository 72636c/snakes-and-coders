/* global $, SpeechSynthesisUtterance, monaco */
...
function processTests (results) {
  ...
  try {
    var testsAsserts = JSON.parse($('textarea#code-editor-tests-asserts').val())
    processTestsAuxiliary(testsAsserts, results.asserts, 'asserts')
  } catch (ignore) {
    removeTests()
  }
  try {
    var testsPrints = JSON.parse($('textarea#code-editor-tests-prints').val())
    processTestsAuxiliary(testsPrints.positives, results.prints.positives, 'prints-positives')
    processTestsAuxiliary(testsPrints.negatives, results.prints.negatives, 'prints-negatives')
  } catch (ignore) {
    removeTests()
  }
}

// Combine status code and message for viewing.
...
function preprocessConfig (config) {
  ...
  try {
    ...
    $.each(randomInts, function (index) {
      ...
      stringifiedConfig = stringifiedConfig.replace(re, randomInt)
    })
  } catch (ignore) {
    return config
  }
...
$('form#code-editor-form').submit(function (event) {
  ...
  var valuesAsserts = []
  try {
    testsAsserts = JSON.parse($('textarea#code-editor-tests-asserts').val())
    $.each(testsAsserts, function (index) {
      valuesAsserts.push(testsAsserts[index].expression)
    })
    testsPrints = JSON.parse($('textarea#code-editor-tests-prints').val())
  } catch (ignore) {
    removeTests()
